...
exports.getAssignments = async (req, res) => {
  try {
    ...
    // Filter by status if provided, otherwise exclude cancelled and closed cases
    if (status && status !== 'all') {
      query = query.eq('status', status);
    } else {
      // By default, don't show cancelled or closed cases
      query = query.not('status', 'eq', 'cancelled').not('status', 'eq', 'closed');
    }
...
exports.markOverdueAssignments = async (req, res) => {
  try {
    ...
    const phtOffset = 8 * 60; // 8 hours in minutes
    const phtTime = new Date(currentTime.getTime() + (phtOffset * 60 * 1000));
    const today = phtTime.toISOString().split('T')[0];
    ...
    res.json({ 
      success: true,
      count: markedCount,
      message: `Marked ${markedCount} assignments as overdue (shift-based deadline check)`
    });
